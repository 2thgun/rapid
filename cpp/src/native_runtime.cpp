...
Runtime::Runtime(Config config)
    : config_(std::move(config)),
      state_(Json::parse(read_file(config_.assets / "state-defaults.json"))),
      store_(config_.database),
      recorder_(
          config_.telemetry, config_.upload_policy,
          [this](const fs::path &path) {
            if (!config_.upload_enabled)
              return;
            auto manifest = Json::parse(read_file(path / "manifest.json"));
            if (!manifest["metadata"].value("upload_after_session", false))
              return;
            Database queue(config_.queue);
            queue.exec(
                "CREATE TABLE IF NOT EXISTS jobs(path TEXT PRIMARY KEY,state "
                "TEXT NOT NULL,error TEXT,updated_at REAL NOT NULL)");
            queue.exec("INSERT INTO jobs VALUES(?,'pending',NULL,?) ON "
  ...
  store_.exec("CREATE TABLE IF NOT EXISTS native_acc_laps(id INTEGER PRIMARY "
              "KEY,session_index INTEGER,car_index INTEGER,lap_number "
              "INTEGER,lap_time_ms INTEGER,is_valid INTEGER,completed_at "
              "TEXT,UNIQUE(session_index,car_index,lap_number))");
  ...
  if (config_.upload_enabled) {
    Database queue(config_.queue);
    queue.exec("CREATE TABLE IF NOT EXISTS jobs(path TEXT PRIMARY KEY,state "
               "TEXT NOT NULL,error TEXT,updated_at REAL NOT NULL)");
    for (const auto &entry : fs::directory_iterator(config_.telemetry))
      if (entry.is_directory() &&
        ...
        if (manifest["metadata"].value("upload_after_session", false))
          queue.exec("INSERT INTO jobs VALUES(?,'pending',NULL,?) ON "
                     "CONFLICT(path) DO NOTHING",
                     {entry.path().string(), double(std::time(nullptr))});
      }
  }
}
void Runtime::sectors(Json &f) {
  int lap = number(f, "lap_number", -1), time = number(f, "current_lap_ms"),
      complete = number(f, "completed_lap_ms");
  auto record = [&](int index, int duration) {
    ...
    f[key + "_ms"] = duration;
    f[key + "_delta_ms"] =
        best_sectors_[index] ? duration - best_sectors_[index] : 0;
    if (!best_sectors_[index] || duration < best_sectors_[index])
      best_sectors_[index] = duration;
  };
  if (lap != timing_lap_) {
  ...
  if (f.contains("lap_position") && splits_.size() < 2) {
    double position = number(f, "lap_position");
    if (position > 1)
      position /= 100;
    if (position >= double(splits_.size() + 1) / 3) {
      int duration = time - (splits_.empty() ? 0 : splits_.back());
      if (duration > 0) {
        record(splits_.size(), duration);
        splits_.push_back(time);
      }
    }
  }
}
bool Runtime::receive(const std::string &payload, const std::string &host) {
  ...
  try {
    ...
    if (config_.companion_host.empty() &&
    ...
    if (m.contains("schema_version") &&
        (!m["schema_version"].is_number_integer() ||
         number(m, "schema_version") < 1 || number(m, "schema_version") > 100))
      throw std::runtime_error("invalid schema version");
    if (m.contains("monotonic_us") &&
        (!m["monotonic_us"].is_number_integer() ||
    ...
    if (type == "status") {
      ...
      for (const auto *key :
           {"track_name", "car_model", "driver_name", "completed_lap_ms"})
        if (!frame.contains(key) && m.contains(key))
          frame[key] = m[key];
      if (frame.contains("abs") && !frame.contains("abs_activity"))
        frame["abs_activity"] = frame["abs"];
      for (auto it = frame.begin(); it != frame.end(); ++it)
        if (live_channels().contains(it.key()) && !it.value().is_null()) {
          bool text = it.key() == "track_name" || it.key() == "car_model" ||
                      it.key() == "driver_name";
          if (text ? !it.value().is_string()
                   : (!it.value().is_number() &&
                      !(it.key() == "pit_limiter" && it.value().is_boolean())))
            throw std::runtime_error("invalid channel type");
        }
      for (const auto *key : {"rpm", "steering_angle", "g_x", "g_y", "g_z"})
        if (!frame.contains(key) || !frame[key].is_number() ||
            !std::isfinite(frame[key].get<double>()))
          throw std::runtime_error("missing primary channel");
      if (number(frame, "rpm") < 0 || number(frame, "rpm") > 20000)
        throw std::runtime_error("invalid RPM");
      for (auto it = frame.begin(); it != frame.end(); ++it) {
        if (it.value().is_number() &&
            (!std::isfinite(it.value().get<double>()) ||
             std::abs(it.value().get<double>()) > 1e12))
          throw std::runtime_error("invalid numeric channel");
      ...
      for (const auto *key : {"gear", "lap_number", "current_lap_ms",
                              "completed_lap_ms", "delta_ms"})
    ...
    state_["companion_daemon_state"] = daemon;
    state_["companion_source_host"] = host;
...
void Runtime::upload(bool enabled) {
  std::lock_guard lock(mutex_);
  recorder_.set_upload(enabled);
  state_["upload_after_session"] = enabled;
}
...
void Runtime::power() {
  ...
  std::uint32_t message[] = {32, 0, 0x30046, 8, 0, 0, 0, 0};
  int fd = ::open("/dev/vcio", O_RDWR | O_CLOEXEC);
  bool ok = false;
  if (fd >= 0) {
    ok = ioctl(fd, _IOWR(100, 0, char *), message) == 0 &&
         (message[1] & 0x80000000U);
    ::close(fd);
  }
  std::lock_guard lock(mutex_);
  state_["power_status_available"] = ok;
  state_["throttled_flags"] = ok ? Json(message[5]) : Json();
  state_["power_limited"] = ok && (message[5] & 7);
  state_["power_limited_since_boot"] = ok && (message[5] & 0x70000);
}
void Runtime::acc(int type, const Json &p) {
  std::lock_guard lock(mutex_);
  store_.exec(
      "INSERT INTO native_acc_packets(received_at,packet_type,normalized_json) "
      "VALUES(?,?,?)",
      {now(), type, p.dump()});
  if (type == 1) {
    state_["acc_connected"] = p.at("success");
    state_["connection_id"] = p.at("connection_id");
  }
  if (type == 2) {
    for (const auto *key : {"session_index", "session_type"})
      state_[key] = p.at(key);
    state_["selected_car_index"] = p.at("focused_car_index");
  }
  if (type == 5)
    state_["track_name"] = p.at("track_name");
  if (type == 6 && p.at("car_index") == state_["selected_car_index"]) {
    state_["car_model"] = std::to_string(p.at("car_model").get<int>());
    int driver = number(p, "current_driver_index");
    if (driver >= 0 && std::size_t(driver) < p.at("drivers").size())
      state_["driver_name"] = string(p["drivers"][driver], "first_name") + " " +
                              string(p["drivers"][driver], "last_name");
  }
  if (type == 3 && p.at("car_index") == state_["selected_car_index"]) {
    state_["gear"] = number(p, "gear_raw") - 1;
    state_["speed_kmh"] = p.at("speed_kmh");
    state_["current_lap_ms"] = p["current_lap"]["lap_time_ms"];
    state_["delta_ms"] = p.at("delta_ms");
    state_["lap_number"] = p.at("laps");
    int completed = number(p["last_lap"], "lap_time_ms");
    if (completed > 0 && completed != 2147483647) {
      state_["completed_lap_ms"] = completed;
      store_.exec("INSERT OR IGNORE INTO "
                  "native_acc_laps(session_index,car_index,lap_number,lap_time_"
                  "ms,is_valid,completed_at) VALUES(?,?,?,?,?,?)",
                  {state_["session_index"], p.at("car_index"), p.at("laps"),
                   completed, p["last_lap"]["valid"], now()});
    }
  }
  if (state_.value("acc_connected", false)) {
    state_["connected"] = true;
    state_["simulator"] = "ACC";
  }
  state_["received_at"] = now();
}
void udp_loop(Runtime &runtime, const Config &config) {
  ...
  addr.sin_port = htons(config.companion_port);
  addr.sin_addr.s_addr = INADDR_ANY;
  if (bind(fd, reinterpret_cast<sockaddr *>(&addr), sizeof addr)) {
    ::close(fd);
    throw std::runtime_error("companion UDP port unavailable");
  }
  while (!stopping) {
    ...
    try {
      ...
      log(std::string("ERROR finalization retry: ") + e.what());
    }
  }
  ::close(fd);
}
