...
std::string string(const Json& j, const std::string& key, const std::string& fallback = "");
...
struct Config {
  std::string host="0.0.0.0", companion_host, acc_host="192.168.1.89", acc_password;
  std::string display_name="raPId", upload_url, upload_token, upload_policy="races";
  int port=8000, companion_port=9001, acc_port=9000, acc_local_port=9000, protocol_version=4, interval_ms=100;
  bool acc_enabled=false, upload_enabled=false;
  fs::path database="data/rapid.db", telemetry="data/telemetry", queue="data/upload-queue.db", assets="cpp/assets";
...
class Recorder {
  ...
  void checkpoint();
  void close();
  fs::path publish(const fs::path& spool, Json state, const std::string& reason);
public:
  Recorder(fs::path root, std::string policy, std::function<void(const fs::path&)> callback={});
  ~Recorder();
...
struct Response { int status=200; std::string body; std::string type="application/json"; std::vector<std::pair<std::string,std::string>> headers{}; };
struct Request { std::string method,target,body; std::map<std::string,std::string> headers; };
using Handler=std::function<Response(const Request&)>;
void serve(const std::string& host, int port, Handler handler, Runtime* runtime=nullptr);
}
