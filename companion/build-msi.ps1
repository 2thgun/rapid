...
& $wix build -arch x64 -d "CompanionExe=$exe" -d "CompanionLauncher=$(Join-Path $PSScriptRoot 'start-rapid-daemon.vbs')" -d "PairLauncher=$(Join-Path $PSScriptRoot 'PAIR.cmd')" -d "ConfigExample=$(Join-Path $PSScriptRoot 'daemon.conf.example')" -o $output (Join-Path $root 'installer\raPIdCompanion.wxs')
