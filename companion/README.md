...
The pairing credential storage primitive is now available in the native
daemon. A paired 256-bit key can be imported into the current user's DPAPI
scope with `--store-auth-key-dpapi PATH` and loaded with
`--auth-key-dpapi-file PATH`; neither operation writes a plaintext key. The
full HTTPS request/approval UI is still pending.
